...
```mermaid
%%{init: {"themeVariables": {"fontSize": "24px"}, "flowchart": {"nodeSpacing": 40, "rankSpacing": 55}}}%%
flowchart TB
  SOURCES["브라우저 내부에서 태스크가 준비되는 계기<br/>HTML 데이터 · 사용자 입력<br/>타이머 만료 · 네트워크 완료"]

  subgraph RENDERER["렌더러 프로세스"]
    THREAD["렌더러 메인 스레드<br/>(OS 스레드 하나)<br/>아래 코드를 한 번에 하나씩 실행"]
    EL["1. 이벤트 루프 구현 코드 실행<br/>태스크 큐 확인·태스크 하나 선택<br/>실행할 태스크가 없으면 대기"]
    TQ["태스크 큐들(자료 구조)<br/>HTML 데이터 전달 · 클릭 이벤트<br/>타이머 · 네트워크 완료 후속 처리"]
    TASK["2. 선택된 태스크 코드 실행<br/>실행 중에는 다음 태스크를 선택하지 않음"]
    PARSER["HTML 파서 코드<br/>HTML 해석 · DOM 생성"]
    JS["JavaScript 엔진 코드<br/>스크립트·콜백의 동기 코드 실행"]
    TASK_RETURN["호출한 파서·태스크 코드로 복귀<br/>나머지 작업 수행 후 태스크 종료"]
    CHECK["3. 마이크로태스크 체크포인트"]
    MQ["마이크로태스크 큐(자료 구조)<br/>Promise.then · await 이후 코드<br/>JavaScript 콜백을 큐가 빌 때까지 실행"]
    RENDER["4. 렌더링 기회라면 관련 코드 실행<br/>스타일 · 레이아웃 · 페인트 준비"]
    RETURN["5. 이벤트 루프 코드로 복귀<br/>다음 반복에서 다시 1번으로"]
  end

  SOURCES -->|"관련 태스크 준비·등록"| TQ
  THREAD -->|"이 스레드에서 실행"| EL
  EL -->|"실행 가능한 태스크 확인"| TQ
  ...
  JS -->|"동기 실행 종료<br/>필요하면 호출한 파서로 복귀"| TASK_RETURN
  TASK_RETURN -->|"태스크가 완전히 끝남"| CHECK
  CHECK -->|"마이크로태스크 확인"| MQ
  MQ -->|"큐가 비면 다음 단계"| RENDER
  RENDER -->|"렌더링 수행 또는 건너뜀"| RETURN
```

> 두 번째 그림의 맨 위 박스는 특정 프로세스나 스레드가 아니라 **브라우저 내부에서 태스크가 준비되는 여러 계기**를 묶어 나타낸 것이다. 태스크를 준비하는 구체적인 내부 구성 요소는 작업 종류와 브라우저 구현에 따라 다르다.
>
> **브라우저 프로세스**는 Chrome 전체가 아니라 Chrome을 구성하는 중심 프로세스 하나다. 모든 태스크를 직접 등록하는 단일 주체라는 뜻은 아니다.
>

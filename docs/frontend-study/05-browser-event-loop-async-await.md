# 브라우저 JavaScript의 이벤트 루프와 `async` / `await`

로그인처럼 서버 응답을 기다리는 작업을 할 때, 브라우저의 JavaScript는 일반적인 Spring MVC + Tomcat 요청 처리와 다르게 동작한다.

## 핵심 결론

브라우저에서 JavaScript를 실행하는 기본 모델은 **이벤트 루프 기반**이다. `fetch` 같은 네트워크 요청은 브라우저의 네트워크 계층에 등록되고, JavaScript 메인 스레드는 서버 응답을 기다리며 멈춰 있지 않는다.

`await`는 스레드를 블로킹하는 명령이 아니다. 현재 `async` 함수의 나머지 실행을 잠시 미루고, 제어권을 이벤트 루프에 돌려준다.

## 메인 스레드와 이벤트 루프

일반적인 웹 페이지의 JavaScript는 브라우저 렌더러 프로세스에 있는 **메인 OS 스레드** 하나에서 실행된다. 이 스레드를 보통 **JavaScript 메인 스레드**라고 부른다.

이벤트 루프는 별도의 스레드가 아니라, 이 메인 스레드에서 다음 JavaScript 작업을 하나씩 꺼내 실행하는 브라우저의 작업 방식이다. 클릭·키보드 입력 이벤트, React의 `onClick`·`onSubmit`, 타이머 콜백, Promise의 후속 코드는 모두 이 메인 스레드에서 실행된다.

한 JavaScript 작업이 실행되는 동안에는 같은 메인 스레드에서 다른 JavaScript 작업이 동시에 실행되지 않는다. 따라서 긴 반복문이나 무거운 계산은 클릭 처리와 화면 갱신까지 막을 수 있다. 현재 실행 중인 JavaScript와 태스크·마이크로태스크의 정확한 관계는 아래의 `setTimeout` 예시에서 설명한다.

그렇다면 어떤 코드는 현재 작업 안에서 바로 실행되고, 어떤 코드는 현재 작업이 끝난 뒤에 실행될까? 실행 시점에 따라 다음처럼 구분할 수 있다.

```text
동기 코드: 현재 호출 스택에서 즉시 실행
사용자 이벤트 처리·타이머 콜백: 태스크(task) 큐에 등록
네트워크 응답 처리: 브라우저가 태스크로 처리하며, fetch Promise가 완료되면 그 후속 코드를 등록
Promise 후속 코드(.then / .catch / .finally, await 다음 코드): 마이크로태스크(microtask) 큐에 등록
```

여기서 태스크는 흔히 “매크로태스크”라고도 부른다. 다만 브라우저에는 작업 출처별 태스크 큐가 있을 수 있어, 보통은 **태스크**라고 부르는 편이 정확하다. 현재 태스크가 끝나면 마이크로태스크를 먼저 모두 처리한 뒤 다음 태스크를 실행한다.

### 왜 두 종류로 나뉘는가?

동기 코드는 지금 필요한 값이 있거나, 다른 함수를 바로 호출하는 코드다. 현재 실행 중인 함수가 `onClose()`처럼 다른 함수를 호출하면, 그 함수는 큐를 거치지 않고 현재 호출 스택에서 즉시 실행된다.

반면 클릭, 타이머, 네트워크 응답은 **언제 발생할지 지금 알 수 없는 외부 사건**이다. 브라우저는 응답이 오기 전에는 그 뒤의 코드를 실행할 수 없으므로, 사건이 발생했을 때 실행할 작업을 큐에 넣어 둔다.

```ts
function handleClick() {
  console.log('1. 지금 실행');

  closePanel(); // 일반 함수 호출: 지금 즉시 실행

  setTimeout(() => {
    console.log('3. 타이머 뒤에 실행');
  }, 0); // 콜백을 등록하고, 실제 실행은 나중에 태스크 큐에서 처리

  console.log('2. 지금 실행');
}
```

### 콜백 함수란?

콜백 함수란, **함수 자체를 다른 함수에 전달해 두고, 그 다른 함수가 정한 시점에 호출하는 함수**입니다.

영어 **callback**은 직역하면 “다시 불러 주기(call back)”입니다. 내가 어떤 함수에 일을 맡기면서 “조건이 갖춰지거나 일이 끝나면, 내가 준 이 함수를 다시 불러 주세요”라고 전달하는 모습에서 나온 이름입니다. 즉, 함수가 자기 자리에서 바로 실행되는 것이 아니라, **다른 주체가 나중에 호출을 되돌려 준다**는 뜻입니다.

`setTimeout` 예시에서는 다음과 같습니다.

```ts
setTimeout(() => {
  console.log('나중에 실행');
}, 0);
```

여기서 `() => { ... }`는 지금 호출하는 것이 아니라 **함수 자체를 전달**한 것입니다. 그리고 그 함수를 실제로 호출하는 주체는 `setTimeout`의 타이머 처리 시스템입니다. 호출 시점도 `setTimeout`의 규칙에 맡기므로 콜백 함수라고 부릅니다.

정리하면:

- `() => {}`: 매개변수 없는 화살표 함수
- `setTimeout(함수, 시간)`: 함수를 전달받아 나중에 실행하는 함수
- 전달한 함수: `setTimeout`이 나중에 호출하므로 콜백 함수

핵심은 화살표 함수 문법이 아니라, **누가 언제 호출하느냐를 전달받은 함수에 맡긴다**는 점입니다.

> **중요 — `setTimeout(..., 0)`은 즉시 실행이 아니다**
>
> **태스크(task)**는 브라우저 이벤트 루프가 선택하여 처리하는 작업 단위이지, JavaScript 함수 하나를 뜻하지는 않는다. 하나의 태스크가 브라우저 내부 작업을 수행하다가 JavaScript 콜백을 호출할 수 있다.
>
> | 태스크의 대표적인 작업 | 태스크가 실행하거나 등록하는 JavaScript |
> | --- | --- |
> | 클릭 이벤트 전달 | 등록된 `click` 이벤트 핸들러를 실행 |
> | 타이머 만료 처리 | `setTimeout` 콜백을 실행 |
> | 네트워크 응답 처리 | `fetch` Promise를 완료하고, `.then` 또는 `await` 다음 코드를 마이크로태스크로 등록 |
> | HTML 파싱 | `async`·`defer`가 없는 일반 `<script>`를 만나면 파싱을 멈추고 해당 스크립트를 실행 |
>
> 태스크나 마이크로태스크가 JavaScript를 실행하기 시작하면, 현재 함수와 그 함수가 동기적으로 호출한 함수들을 **호출 스택이 빌 때까지 중단하지 않고 실행**한다. 이 실행 중에는 다른 태스크의 콜백뿐 아니라 이미 등록된 **마이크로태스크도 중간에 끼어들 수 없다.** 이것이 **run-to-completion**이다.
>
> 위 예시는 다음 순서로 실행된다.
>
> ```text
> 클릭 태스크에서 handleClick() 실행
>   → '1. 지금 실행' 출력
>   → closePanel()을 같은 호출 스택에서 즉시 실행
>   → setTimeout 콜백을 나중에 실행하도록 등록
>   → '2. 지금 실행' 출력
>   → handleClick() 종료, 호출 스택이 빔
>
> 등록된 마이크로태스크가 있다면 먼저 모두 실행
>   → 이 예시에는 등록된 마이크로태스크가 없음
>
> 타이머 태스크가 선택되면 setTimeout 콜백 실행
>   → '3. 타이머 뒤에 실행' 출력
> ```
>
> 따라서 실행 순서는 `1 → closePanel() 내부 코드 → 2 → 3`이다. `setTimeout(..., 0)`의 `0`은 콜백을 지금 실행하라는 뜻이 아니라 타이머에 요청한 지연값이다. 현재 동기 실행과 마이크로태스크 처리가 끝난 뒤 이벤트 루프가 타이머 태스크를 선택해야 콜백이 실제로 실행된다.

### `async`, `await`, `fetch`는 어떻게 함께 동작하는가?

```ts
async function load() {
  console.log('1. 요청 시작');

  const response = await fetch('/api/users');

  console.log('3. 응답 뒤 실행');
  return response;
}

console.log('0');
const loadPromise = load();
console.log('2');
```

`async`가 붙은 `load`는 비동기 함수다. 하지만 `async`는 이 함수 전체를 백그라운드에서 실행한다는 뜻이 아니다. **`await`를 사용할 수 있게 하고, 함수의 최종 결과를 Promise로 반환하게 하는 표시**다.

따라서 `load()`는 `await`를 만나기 전까지 일반 함수처럼 즉시 실행된다. `await`가 아직 완료되지 않은 Promise를 만나면, 그 시점부터 `load`의 나머지 코드가 보류되고 호출자에게 Promise와 함께 제어권이 돌아간다.

`await` 다음의 코드는, 해당 Promise가 완료될 때까지 **그 `async` 함수 안에서 실행이 보류된다.** 여기서 “보류”는 JavaScript 전체가 멈추는 블로킹이 아니다. `load` 함수의 `await` 뒤쪽 코드만 나중으로 미뤄질 뿐, 호출자 코드나 다른 이벤트 처리, 화면 갱신 등은 계속 진행될 수 있다.

응답이 도착해 Promise가 완료된 뒤에는 현재 동기 코드가 끝난 후 `load`가 재개되어 `3`이 출력된다.

실행 순서: `0 → 1 → 2 → 3`

## 네트워크 I/O는 메인 스레드가 기다리지 않는다

`fetch`를 호출하는 JavaScript 코드는 메인 스레드에서 실행되지만, 실제 HTTP 연결·패킷 송수신·응답 대기는 브라우저의 네트워크 계층과 OS가 처리한다.

```text
메인 스레드
  fetch('/api/login') 호출 → 네트워크 요청 등록 → Promise를 즉시 받음

브라우저 네트워크 계층 / OS
  HTTP 연결·송수신·응답 대기 수행

메인 스레드
  응답 도착 후 Promise 후속 JavaScript 코드 실행
```

즉, `fetch`의 요청 시작 코드와 응답 뒤 실행할 코드는 메인 스레드에서 실행하지만, 네트워크 응답을 기다리는 동안 메인 스레드가 대기 상태로 점유되지는 않는다.

서버 응답이 돌아온 뒤에는 다음 역할이 순서대로 이어진다.

```text
OS
  → 브라우저의 네트워크 계층에 수신 가능한 데이터가 있음을 알림
  → 브라우저의 메인 스레드에 CPU 실행 시간을 배정

브라우저 네트워크 계층
  → 소켓에서 응답 바이트 수신
  → HTTP 응답으로 해석
  → fetch Promise 완료

브라우저 이벤트 루프
  → await 뒤 코드를 재개할 마이크로태스크를 등록
  → 현재 작업이 끝난 뒤 마이크로태스크를 선택

JavaScript 엔진
  → 선택된 코드, 즉 await 다음 줄부터 실행
```

### 예시: 로그인 응답 후 `await` 다음 줄이 실행되는 과정

```tsx
async function handleSubmit() {
  console.log('1. 로그인 요청 시작');

  const response = await fetch('/api/login');
  if (!response.ok) {
    throw new Error('로그인 실패');
  }

  console.log('2. 서버 응답 수신 후 실행');
  onClose();
}
```

```text
1. 로그인 버튼 클릭으로 handleSubmit()이 호출된다.
2. fetch가 Spring Boot 서버로 HTTP 요청을 등록하고 Promise를 반환한다.
3. await는 현재 handleSubmit 호출을 잠시 중단한다. 메인 스레드는 다른 작업을 처리할 수 있다.
4. Spring Boot가 HTTP 응답을 전송한다.
5. 브라우저 네트워크 계층이 응답을 해석하고 fetch Promise를 완료한다.
6. await 다음 코드를 재개할 마이크로태스크가 큐에 등록된다.
7. 현재 JavaScript 작업이 끝나면, 같은 handleSubmit 호출이 await 다음 줄부터 재개된다.
8. response.ok가 true이면 '2. 서버 응답 수신 후 실행'을 출력하고 onClose()를 호출한다.
```

`await`는 잠금(lock)을 잡고 있는 것이 아니다. 로그인 성공 후 `handleSubmit()`을 새로 호출하는 것도 아니다. Promise가 완료되면 이미 실행 중이었다가 중단된 **같은 `handleSubmit` 호출**이 이어서 실행된다.

`fetch`는 HTTP `401`, `404`, `500` 응답을 받았을 때도 네트워크 통신 자체는 성공했으므로 `Response`를 반환할 수 있다. 따라서 로그인 성공 여부는 `response.ok`를 확인해 판단한다.

## Spring MVC + Tomcat과 비교

| 구분 | Spring MVC + Tomcat | 브라우저 JavaScript + `fetch` |
|---|---|---|
| 요청 처리 | 보통 요청마다 Tomcat 작업 스레드가 배정된다. | JavaScript 메인 스레드가 이벤트를 하나씩 처리한다. |
| 외부 I/O 대기 | JDBC나 외부 HTTP를 동기로 호출하면 해당 스레드가 대기한다. | 요청을 브라우저 네트워크 계층에 등록하고 즉시 `Promise`를 받는다. |
| 응답 도착 후 | 대기 중이던 스레드가 이어서 실행한다. | 이벤트 루프가 후속 JavaScript 코드를 큐에서 꺼내 실행한다. |
| 스레드 점유 | 일반적인 동기 MVC 코드에서는 스레드가 점유될 수 있다. | 네트워크 대기 동안 JavaScript 메인 스레드는 점유하지 않는다. |

백엔드가 Spring MVC + Tomcat으로 처리되어 서버 스레드가 대기하더라도, 그것은 서버 측의 일이다. 브라우저의 JavaScript 메인 스레드는 서버 응답을 기다리며 멈추지 않는다.

## `Promise<void>`의 의미

```ts
Promise<void>
```

- `Promise`: 성공 또는 실패로 나중에 한 번 완료될 비동기 작업이다.
- `<void>`: Java의 제네릭처럼, 작업이 성공한 뒤 제공할 결과값의 타입을 적는 자리다.
- `void`: 성공해도 호출자에게 돌려줄 의미 있는 값이 없다는 뜻이다.

따라서 `Promise<void>`는 “로그인 작업이 끝날 때까지는 기다릴 수 있지만, 완료 후 `LoginForm`이 받을 결과값은 없다”는 뜻이다. Java에서는 `CompletableFuture<Void>`와 가장 비슷하다.

로그인 성공 후 사용자 정보와 로그인 상태는 보통 부모의 `useAuth`가 관리한다. `LoginForm`은 로그인 완료만 기다린 뒤 `onClose()`를 호출하면 된다.

## WebFlux + Netty와의 관계

브라우저의 비동기 I/O 흐름은 WebFlux + Netty의 이벤트 루프와 비슷하게 이해할 수 있다.

```text
Netty 이벤트 루프
  비차단 I/O 감시 등록 → 다른 작업 처리 → I/O 준비됨 → 후속 처리 실행

브라우저 이벤트 루프
  fetch 요청을 네트워크 계층에 맡김 → 다른 작업 처리
  → 응답 도착 → Promise 후속 코드를 마이크로태스크 큐에서 실행
```

둘 다 “I/O 응답을 기다리는 동안 처리 스레드를 붙잡아 두지 않는다”는 공통점이 있다. 다만 `Promise`는 한 번 성공 또는 실패로 완료되는 객체이고, `Mono`와 `Flux`는 Reactive Streams의 Publisher라는 점에서 정확히 같은 개념은 아니다.

### `CompletableFuture`와 `Mono`를 가장 쉽게 구분하는 법

```text
CompletableFuture
= 이미 시작한 작업의 결과를 나중에 받는 대기표

Mono
= 구독되면 시작할 작업 흐름의 설계도
```

```text
CompletableFuture.supplyAsync(...)
  → 보통 즉시 작업 시작
  → 나중에 결과 도착

Mono.fromSupplier(...)
  → 아직 작업 시작 안 함
  → subscribe()되면 작업 시작
  → 나중에 결과 도착
```

한 줄로 정리하면, `CompletableFuture`는 **진행 중인 작업의 완료 결과**이고 `Mono`는 **실행 대기 중인 작업 흐름**이다. 단, `new CompletableFuture<>()`처럼 직접 만들어 나중에 완료시키는 방식도 있으므로, 위 비교는 `supplyAsync()`를 사용하는 일반적인 경우다.

- `Promise`: 보통 호출 즉시 작업이 시작되며, 한 번만 완료 또는 실패한다.
- `Mono`: 최대 한 개의 값을 발행한다. 일반적으로 구독되었을 때 실행되는 cold publisher로 사용할 수 있다.
- `Flux`: 여러 값을 시간에 따라 계속 발행한다. 이 프로젝트의 SSE 호가 스트림이 이에 더 가깝다.

## 주의: 모든 JavaScript가 자동으로 논블로킹인 것은 아니다

`fetch`는 브라우저가 제공하는 비동기 네트워크 I/O이므로 메인 스레드를 막지 않는다. 하지만 JavaScript에서 오래 걸리는 계산을 직접 수행하면 메인 스레드가 점유되어 화면도 멈춘다.

```ts
while (true) {
  // 메인 스레드를 계속 점유하므로 클릭·렌더링도 처리할 수 없다.
}
```

즉, `async` / `await`가 자동으로 새 스레드를 만드는 것은 아니다. 비동기 I/O 작업의 완료를 나중에 이어서 처리하도록 만드는 문법이며, 긴 CPU 작업은 별도로 나누거나 Web Worker 같은 방법을 고려해야 한다.

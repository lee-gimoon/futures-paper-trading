...
`root.render(<App />)`가 최초 렌더링을 요청하면 React가 App을 호출하고, App은 render 중 Hook을 호출한 뒤 JSX를 반환한다. 그 결과가 commit되어 JSX에 해당하는 UI가 브라우저 DOM에 처음 반영되면, App의 **mount(마운트)**가 완료된다.

...
| **마이크로태스크 체크포인트** | 선택된 태스크가 끝난 뒤 마이크로태스크 큐를 확인하는 절차다. 큐가 비어 있으면 바로 통과하고, 비어 있지 않으면 새로 추가되는 것까지 큐가 빌 때까지 처리한다. 태스크 종료 외에도 표준이 정한 시점에 수행될 수 있다. |
...
flowchart TB
  ...
  subgraph RENDERER["렌더러 프로세스"]
    ...
    MQ["마이크로태스크 큐<br/>Promise.then · await 이후 코드<br/>있으면 큐가 빌 때까지 처리"]

    subgraph MAIN["렌더러 메인 스레드(OS 스레드 하나)"]
      EL["① 렌더러 메인<br/>스레드가 이벤트<br/>루프 구현 코드 실행<br/>태스크 큐 확인·선택"]
      WAIT["실행 가능한<br/>태스크가 없으면<br/>렌더러 메인<br/>스레드 대기"]
      TASK["② 같은 렌더러 메인<br/>스레드가 선택된<br/>태스크 코드 실행<br/>실행 중에는 다음<br/>태스크 선택 안 함"]
  ...
  TASK_RETURN -->|"태스크 종료"| CHECK
  CHECK -->|"있음"| MQ
  MQ -->|"JavaScript 콜백 실행"| JS
  JS -->|"마이크로태스크 실행 종료"| CHECK
  CHECK -->|"비어 있음"| RENDER
  RENDER -->|"수행 또는 건너뜀"| RETURN
  RETURN -->|"①부터 반복"| EL
```

> **주 흐름:** ① 렌더러 메인 스레드가 이벤트 루프 구현 코드 실행·태스크 선택 → ② 같은 렌더러 메인 스레드가 선택된 태스크 코드 실행 → ③ 태스크 종료 → ④ 마이크로태스크 체크포인트 → ⑤ 필요하면 렌더링 → ⑥ 이벤트 루프로 복귀
>
> 태스크가 끝나면 **마이크로태스크 체크포인트 자체는 수행**한다. 이 말은 마이크로태스크를 반드시 실행한다는 뜻이 아니다. 큐가 비어 있으면 확인만 하고 다음 단계로 넘어가며, 큐에 마이크로태스크가 있으면 새로 추가되는 것까지 큐가 빌 때까지 처리한다. 체크포인트는 태스크 종료 외에도 HTML 표준이 정한 시점에 수행될 수 있다.
>

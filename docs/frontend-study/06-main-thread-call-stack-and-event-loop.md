...
Netty의 `EventLoop.run()` 설명에 대응해 브라우저 흐름을 단순화하면 다음과 같다. 브라우저 이벤트 루프는 표준상의 처리 모델이므로 JavaScript에서 보이는 `EventLoop.run()` 메서드가 있다는 뜻은 아니다.
...
#### **이벤트 루프와 렌더러 메인 스레드의 역할**

**이벤트 루프는 실행 가능한 태스크의 선택·실행과 마이크로태스크 체크포인트 등의 처리 순서를 정의한 모델이다. 일반적인 Chromium 페이지에서는 렌더러 메인 스레드가 브라우저의 이벤트 루프 구현 코드를 반복 실행한다. 구현 코드는 실행할 태스크를 선택하고 필요한 처리 단계를 호출하며, 선택된 메인 스레드 태스크의 동기 실행 경로에서 호출되는 Blink·V8·JavaScript 콜백 코드도 같은 렌더러 메인 스레드가 실행한다. 콜 스택은 V8이 현재 실행 중인 JavaScript 함수 호출을 관리하는 실행 상태이며, 코드를 실행하는 별도 스레드가 아니다. Blink·V8의 백그라운드 작업과 합성·래스터 작업까지 모두 메인 스레드가 실행한다는 뜻은 아니다.**

...
import com.example.futurespapertrading.paper.service.PaperOrderService;
...
import org.springframework.web.bind.annotation.DeleteMapping;                     // HTTP DELETE 요청을 이 메서드에 매핑
import org.springframework.web.bind.annotation.GetMapping;                        // HTTP GET 요청을 이 메서드에 매핑
import org.springframework.web.bind.annotation.PathVariable;                      // URL 경로의 {id} 자리 값을 매개변수로 받음
...
import com.example.futurespapertrading.auth.domain.User;                          // email로 조회한 유저 엔티티 (id를 꺼내려고)
import com.example.futurespapertrading.auth.repository.UserRepository;                // email → User 조회 (내 user_id 얻기)
...
// 모의 주문 HTTP 입구. 엔드포인트 3개 — POST /api/paper/orders(시장가·지정가 주문 생성) / GET /api/paper/orders(내 주문 목록) / DELETE /api/paper/orders/{id}(주문 취소).
...
public class PaperOrderController {
    ...
    // DELETE /api/paper/orders/{id} — 주문 취소. 성공 시 200 + status=CANCELED인 OrderResponse.
    //   · @DeleteMapping("/{id}") = 클래스 공통 경로 뒤에 /{id}가 붙은 DELETE 요청 매핑. {id}는 경로 템플릿 변수.
    //   · @PathVariable Long id = URL의 {id} 자리 문자열("…/orders/7"의 "7")을 Long으로 변환해 받음. (본문이 없는 DELETE라 @RequestBody 대신 경로로 식별)
    //   · 검증(404 없음 / 403 남의 것 / 409 OPEN 아님)은 서비스가 도메인 예외로 던지고 PaperExceptionHandler가 상태코드로 번역.
    //   · 진짜 DELETE(행 삭제)가 아니라 status를 CANCELED로 바꾸는 UPDATE다 — 주문은 기록(원장)이라 지우지 않고 남긴다.
    //     응답에 취소된 주문을 담는 것도 그래서 가능(바이낸스 실제 API도 DELETE에 취소된 주문 객체를 돌려준다).
    @DeleteMapping("/{id}")
    public Mono<OrderResponse> cancel(@PathVariable Long id) {
        return currentUserId().flatMap(userId -> orderService.cancel(id, userId));
    }

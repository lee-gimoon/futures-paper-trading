...
`useEffect`와 이벤트 핸들러는 모두 API 요청처럼 컴포넌트 밖의 작업을 실행할 수 있다. 차이는 무엇이 실행을 시작시켰는가다.

- `useEffect`: 화면이 렌더링되었거나, 의존성 값이 바뀌었기 때문에 실행한다.
- 이벤트 핸들러: 사용자가 클릭·입력·제출했기 때문에 실행한다.

#### 초기 세션 확인은 화면이 나타났기 때문에 실행한다

앱을 새로 열었을 때는 사용자가 로그인 버튼을 누르지 않아도, 브라우저에 이전 SESSION 쿠키가 남아 있을 수 있다. 따라서 화면이 처음 나타난 뒤 현재 로그인한 사용자가 있는지 서버에 확인해야 한다.

```tsx
useEffect(() => {
  authApi.fetchMe(); // GET /api/auth/me
}, []);
```

이 요청의 이유는 “사용자가 방금 어떤 행동을 했다”가 아니라 “앱이 처음 화면에 나타났다”이므로 `useEffect`에 둔다.

#### 로그인 요청은 사용자가 제출했기 때문에 실행한다

```tsx
async function handleSubmit(e: FormEvent) {
  e.preventDefault();
  await onLogin(email, password); // POST /api/auth/login
}
```

이 요청은 사용자가 로그인 form을 제출했을 때만 실행되어야 한다. 그래서 `handleSubmit` 이벤트 핸들러에서 바로 시작한다.

로그인 요청을 Effect에 넣으면, 제출 핸들러는 요청을 시작하는 대신 “로그인 요청이 필요함”이라는 state만 변경하게 된다. 그러면 실제 요청이 어떤 사용자 행동 때문에 시작됐는지가 state 변화 뒤에 숨고, 의존성 값 변경에 따라 Effect가 다시 실행되지 않도록 추가 제어도 필요해진다.

따라서 이 프로젝트에서는 다음처럼 역할을 나눈다.

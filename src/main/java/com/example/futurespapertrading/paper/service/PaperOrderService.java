...
//   PaperOrderController는 placeOrder·listOrders·cancel을, PendingOrderMatcher는 matchOpenOrders를 호출해 이 서비스 로직을 쓴다.
//   체결 판정·상태 결정·주문/fill 저장을 한곳에서 처리한다.
//
...
public class PaperOrderService {
    ...
    private Mono<OrderResponse> placeOrderChecked(CreateOrderRequest req, Long userId, int leverage) {
        ...
        //   · 왜 저장 전에 probe로 먼저 계산하나: DB에 저장할 최종 상태(FILLED/OPEN/REJECTED)와 filledQuantity를 체결 결과로 결정해야 하기 때문이다.
        //     시장가는 체결되면 FILLED, 체결 0건이면 REJECTED이고, 지정가는 전부 체결되면 FILLED, 일부·미체결이면 OPEN이다.
        //   · 왜 이름이 probe('떠보다')인가: '지금 호가에 이 주문을 대보면 어떻게 체결되나'를 시험하는 계산용 입력이라서 — 저장본이 아니다.

...
| **렌더러 내부 스케줄러** | 전달받은 입력·네트워크 결과와 렌더러 내부의 타이머 같은 작업을 `TaskRunner`를 통해 적절한 태스크 큐에 게시하고 실행 우선순위를 관리한다. |
...
flowchart TB
  ...
  SOURCES -->|"관련 신호·데이터 전달"| SCHEDULER
  SCHEDULER -->|"TaskRunner에 태스크 게시"| TQ
  EL -->|"실행 가능한 태스크 확인"| TQ
  TQ -->|"실행할 태스크 없음"| WAIT
...
> 두 번째 그림의 맨 위 박스는 특정 프로세스나 스레드가 아니라 **태스크가 준비되는 여러 계기**를 묶어 나타낸 것이다. Chrome에서 사용자 입력은 브라우저 프로세스가 렌더러로 전달하고, 네트워크 서비스는 응답 데이터와 완료 사실을 전달하며, 타이머는 렌더러 내부에서 관리된다. 이를 받은 렌더러 내부 코드가 필요한 작업을 `TaskRunner`에 태스크로 게시한다.
>
...
>
> 렌더러 메인 스레드의 초기화는 한 번만 수행된다. 이후에는 태스크 대기·선택·실행과 이벤트 루프로의 복귀가 반복된다. 그림에서 태스크 큐로 향하는 화살표는 **태스크 등록 또는 큐 확인**, 태스크 큐에서 실행 코드로 향하는 화살표는 **선택 결과에 따른 제어권 이동**을 뜻한다. 태스크 큐와 이벤트 루프는 별도 스레드가 아니다.

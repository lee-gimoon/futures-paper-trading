...
**이벤트 루프는 태스크·마이크로태스크·렌더링의 처리 순서를 정의한 모델이다.**<br>
**브라우저 내부의 이벤트 루프 구현 코드는 실행 가능한 태스크를 선택해 처리 함수를 호출하고, 태스크가 끝나면 마이크로태스크 체크포인트를 수행하며, 렌더링 기회를 확인하는 과정을 반복한다.**<br>
**이 구현 코드와 구현 코드가 호출한 태스크 처리 코드 모두 렌더러 메인 스레드가 실행한다.**

...
[O] 6단계: 캔들 차트 (과거 봉 kline REST + 진행 봉 bestAsk)
[O] 7단계: 자체 회원가입 / 로그인
[O] 8단계: 호가창 기준 모의 주문 (백엔드 + 9단계와 함께 프론트 주문 폼)
[O] 9단계: 계좌, 포지션, PnL (+ 거래 패널 프론트)
[ ] 10단계: 운영성 보강
...
- ~~레버리지 / 마진 / 청산~~: → 9단계에서 당겨 구현 완료(격리·MMR0 MVP — 매수가능 검증·% 바·자동 강제청산). 자세히는 backend/stage9-portfolio.md.

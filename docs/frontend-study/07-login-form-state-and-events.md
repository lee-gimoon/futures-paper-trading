...
React는 이 반환값을 받아 이전 렌더링 결과와 비교한 뒤, 필요한 실제 DOM만 생성·수정합니다.

예를 들어 사용자가 이메일을 입력하면:
...
| `submitting` | `false` | `setSubmitting` | 버튼 비활성화와 `로그인 중...` 문구 | 네트워크 요청 중이라는 시간에 따른 상태를 보여 줘야 함 |
...
렌더링 B의 submitting = true
  └─ 새 JSX에서 버튼 disabled=true, 문구='로그인 중...'
```
...
따라서 버튼은 비활성화되고 문구는 `로그인 중...`으로 바뀐다.
...
중복 state가 많으면 서로 맞지 않는 상태가 생길 수 있다. 예를 들어 `submitting=false`인데 `buttonText='로그인 중...'`인 모순이 가능해진다. 현재 코드는 하나의 원천에서 두 UI 값을 계산하므로 그런 모순을 피한다.

---

## 15. mount와 unmount: LoginForm이 생기고 사라지는 시점

### 용어 먼저

- **mount(마운트)**: React가 컴포넌트를 처음 렌더 트리에 넣고, 그 컴포넌트가 설명한 DOM을 화면에 만드는 일이다. 이때 `useState`의 초기값이 처음 사용되고, `useEffect`가 있다면 화면 반영 후 실행된다.
- **unmount(언마운트)**: React가 컴포넌트를 렌더 트리에서 완전히 빼고, 그 컴포넌트가 만든 DOM을 화면에서 제거하는 일이다. 그 컴포넌트의 지역 state는 버려지고, `useEffect` cleanup이 있다면 이때 실행된다.
- **re-render(리렌더)**: 이미 mount되어 있는 컴포넌트 함수를 다시 실행하여 새 JSX를 계산하는 일이다. 컴포넌트가 트리에 계속 남아 있다면 unmount되지 않았으므로 기존 state도 유지된다.

즉 mount/unmount는 컴포넌트가 **생기고 완전히 사라지는 것**이고, re-render는 컴포넌트가 **계속 있는 채로 화면만 다시 계산하는 것**이다. 페이지 새로고침과는 다른 개념이다.

개발자가 `mount()`나 `unmount()`를 직접 호출하지 않는다. JSX의 조건 결과에 따라 React가 자동으로 처리한다.

| 구분 | 뜻 | 현재 LoginForm에서 일어나는 일 |
|---|---|---|
| mount | 컴포넌트를 트리에 새로 추가 | `form === 'login'`이 처음 `true`가 되면 `<LoginForm />`이 생성되고 네 `useState`가 초기화된다. |
| re-render | 이미 있는 컴포넌트의 화면을 다시 계산 | 이메일을 입력하거나 `submitting`이 바뀌면 `LoginForm`은 다시 렌더링되지만, 같은 컴포넌트이므로 state는 유지된다. |
| unmount | 컴포넌트를 트리에서 제거 | `form === 'login'`이 `false`가 되면 `LoginForm`이 제거되고 그 안의 지역 state도 함께 버려진다. |

### 현재 프로젝트에서 LoginForm은 어디에서 쓰이나?

진입점인 `frontend/src/main.tsx`가 `<App />`을 React root에 렌더링한다. 그다음 `frontend/src/App.tsx`가 `LoginForm`을 import하고, `form` state에 따라 필요한 순간에만 렌더링한다.

```text
main.tsx
  └─ root.render(<App />)
       └─ App.tsx
            └─ form === 'login'일 때만 <LoginForm />
                 └─ auth/components/LoginForm.tsx
```

`App.tsx`의 관련 코드는 다음과 같다.

```tsx
// null이면 폼을 열지 않고, 'login'이면 로그인 폼을 연다.
...
여기서 `&&`의 왼쪽 조건이 `true`이면 오른쪽의 `<LoginForm />`이 JSX에 포함된다. `false`이면 React는 아무것도 렌더링하지 않는다. 즉 이 한 줄이 LoginForm의 mount와 unmount를 결정한다.
...
setForm('login')
  ↓ App re-render
form === 'login'이 true
  ↓ <LoginForm />이 JSX에 포함됨
LoginForm mount
  ↓
email='', password='', error='', submitting=false
```

이 최초 mount에서만 `useState('')`, `useState(false)`의 초기값이 사용된다. 그 후 input에 글자를 입력해도 mount가 다시 일어나는 것이 아니라 re-render만 일어나므로 입력한 state는 유지된다.

### 폼을 닫거나 로그인에 성공할 때: unmount

취소 버튼과 성공 경로 모두 `onClose`를 호출한다. `App`이 전달한 `onClose`의 실제 내용은 `setForm(null)`이다.

```text
LoginForm 안에서 onClose()
  ↓ App의 setForm(null)
  ↓ App re-render
form === 'login'이 false
  ↓ <LoginForm />이 JSX에서 빠짐
LoginForm unmount
  ↓
email, password, error, submitting 지역 state도 제거
```

로그인 성공 시에는 `handleSubmit`이 `await onLogin(email, password)`로 로그인 작업이 성공한 뒤 `onClose()`를 호출한다. 따라서 인증 화면으로 바뀌면서 로그인 폼도 사라진다.

### 다시 열면 왜 입력값이 비어 있는가?

닫힌 폼을 다시 열 때 React가 예전 컴포넌트를 보여 주는 것이 아니다. 새 `LoginForm`을 다시 mount하므로 네 state는 다시 초기값으로 시작한다.

```text
닫기: LoginForm unmount → 기존 email/password state 제거
다시 열기: LoginForm mount → email='', password=''로 새로 시작
```

반대로 조건부 렌더링으로 제거하지 않고 CSS의 `display: none` 등으로 숨기기만 했다면 컴포넌트는 같은 트리 위치에 남는다. 이 경우 unmount되지 않으므로 state도 보통 유지된다.

### useEffect의 cleanup과 unmount

현재 `LoginForm`에는 `useEffect`가 없으므로 unmount될 때 정리할 외부 연결도 없다. 다만 `useEffect`가 외부 작업을 시작했다면 `return`으로 cleanup 함수를 작성할 수 있다.
...
useEffect(() => {
  ...
  return () => {
    eventSource.close(); // 컴포넌트가 unmount될 때 실행
  };
}, []);
```

실제 프로젝트의 `useOrderBookStream`은 이 방식으로 SSE 연결을 열고, 사용하는 컴포넌트가 unmount될 때 연결을 닫는다. React state는 코드 줄 자체가 아니라 렌더 트리의 컴포넌트 위치에 연결된다는 점이 핵심이다.

...
예를 들어 React는 npm 패키지로 설치하는 UI 라이브러리이고, Vite도 npm 패키지로 설치하지만 역할은 개발·빌드 도구이다. Hermes는 앱에 포함되는 JavaScript 엔진이며 앱 코드에서 가져다 쓰는 라이브러리가 아니다. Expo Go는 휴대폰 앱이고 EAS는 인터넷에서 제공되는 서비스이므로 npm 모듈이 아니다.
...
| Vite | 사용 | 웹 개발 서버·빌드 도구 | 예: `vite`를 개발 의존성으로 직접 설치 | 개발 서버를 실행하고 TypeScript/TSX를 처리한다. 배포할 때는 웹 파일을 빌드한다. |

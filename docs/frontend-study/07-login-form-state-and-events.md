...
따라서 로그인 API 요청은 `handleSubmit`에서 바로 시작한다. 이를 Effect로 옮기면 “로그인 요청이 필요함”이라는 state를 만들고 Effect가 이를 감시해야 하므로, 요청의 원인인 사용자 제출이 state 변화 뒤에 숨고 중복 실행을 막을 제어도 필요해진다.

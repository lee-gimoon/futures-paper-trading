...
Expo CLI는 `expo` 명령을 처리하는 프로그램이다. `expo start`를 받으면 다음 순서로 개발 실행을 준비한다.

```text
expo start 실행
    ↓
Expo CLI가 현재 mobile/ 폴더를 Expo 프로젝트로 인식
    ↓
app.json에서 앱 이름·화면 방향·Android 앱 식별자 같은 설정 확인
    ↓
개발자 PC에서 Metro 개발 서버 시작
    ↓
휴대폰이 접속할 서버 주소를 QR 코드로 표시
```

Expo CLI가 `app.json`을 읽는 이유는 JavaScript 화면 코드만으로는 이 앱의 이름, 화면 방향, 주소 체계와 Expo 플러그인 같은 **앱 자체의 설정**을 알 수 없기 때문이다. Expo CLI는 이 설정을 확인해 현재 프로젝트의 개발 서버를 올리고, Expo Go에 앱 정보와 접속 정보를 제공할 준비를 한다.
...
`_layout.tsx`와 `index.tsx`는 Expo Router가 정해 둔 예약 파일명이다. `_layout.tsx`여야 모든 화면의 이동 구조를 설정하는 레이아웃으로 인식하고, `index.tsx`여야 그 폴더의 기본 경로(`/`)로 인식하므로 이름을 바꾸면 레이아웃·첫 화면 역할이 달라진다.
...
### Expo Router의 파일 기반 라우팅: 파일명과 화면이 연결되는 방식

Expo Router는 `app/` 파일명을 읽어 내부 주소와 화면을 연결하고, `router.push()` 같은 화면 이동 요청을 처리하는 라우팅 소프트웨어다. 휴대폰 앱에는 웹 브라우저처럼 주소창이 보이지 않지만, 화면을 구분하는 내부 주소가 있다. Expo Router는 주소 목록을 별도 코드로 작성하는 대신 `app/` 폴더의 파일명으로 주소와 화면을 연결한다.

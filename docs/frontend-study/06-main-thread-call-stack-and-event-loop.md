...
> **핵심: 이벤트 루프도 코드이므로 스레드에서 실행된다. 일반적인 브라우저 페이지에서는 별도의 이벤트 루프 스레드가 아니라 렌더러 메인 스레드가 이벤트 루프 코드와 JavaScript를 번갈아 실행한다.**

## 가장 중요한 구분

- **이벤트 루프**: 어떤 작업을 언제 처리할지 정의한 처리 모델이다. 브라우저는 이 모델을 C++ 등의 내부 코드로 구현한다.
- **메인 스레드**: 이벤트 루프 코드와 선택된 JavaScript 작업 등을 실제로 실행하는 OS 스레드다.
- **콜 스택**: JavaScript 엔진이 현재 실행 중인 함수 호출과 실행 상태를 관리하는 구조다.

여기서 `이벤트 루프`라는 말은 문맥에 따라 두 가지를 함께 가리킨다.

1. HTML 표준에 정의된 작업 선택·마이크로태스크 처리·렌더링 기회 등의 **처리 규칙**
2. 그 규칙을 실제로 수행하도록 브라우저 내부에 작성된 **구현 코드**

규칙 자체는 추상적인 개념이지만, 그 규칙을 구현한 코드는 반드시 어떤 스레드에서 실행되어야 한다. 일반적인 페이지에서는 그 스레드가 **렌더러 메인 스레드**다.
...
세 개념은 같은 종류가 아니다. 이벤트 루프는 처리 모델과 그 구현 코드, 메인 스레드는 그 코드를 실제로 실행하는 주체, 콜 스택은 JavaScript 실행 상태를 관리하는 구조다. JavaScript가 메인 스레드를 오래 점유하면 이벤트 루프 코드로 돌아오지 못하므로 다음 태스크, 클릭 처리, 마이크로태스크 체크포인트, 화면 갱신도 함께 늦어진다.

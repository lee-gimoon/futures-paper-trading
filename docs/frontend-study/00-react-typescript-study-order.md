...
## `package.json`, `package-lock.json`, `node_modules`의 관계

세 가지는 이름이 비슷해 보이지만 역할이 다릅니다.

```text
package.json       필요한 라이브러리와 실행 명령의 목록
package-lock.json  실제로 선택된 정확한 라이브러리 버전 목록
node_modules       실제 설치된 라이브러리 파일들
```

### `package.json`: 프로젝트의 목록과 실행 방법

현재 `frontend/package.json`에는 아래처럼 적혀 있습니다.
...
여기서 알 수 있는 내용은 다음과 같습니다.

- `react`, `react-dom`: React를 사용한다.
- `typescript`: TypeScript를 사용한다.
- `vite`: Vite를 사용한다.
- `npm run dev`: `vite`를 실행한다.
- `npm run build`: TypeScript 검사(`tsc`) 후 Vite 빌드를 실행한다.

`package.json`이 없고 `node_modules`만 남아 있어도, 현재 PC에서는 Vite 실행 파일을 직접 찾아 실행할 수는 있습니다.

```powershell
.\node_modules\.bin\vite
```

하지만 `npm run dev`는 `package.json`의 `scripts.dev`에서 무엇을 실행할지 찾으므로, 이 파일이 없으면 실행할 수 없습니다. 또한 새 PC나 다른 사람이 프로젝트를 받았을 때 `node_modules`는 보통 Git에 올리지 않으므로, 어떤 라이브러리를 다시 설치해야 하는지도 알 수 없습니다.

### `package-lock.json`: 정확한 설치 결과

`package.json`의 버전은 어느 정도 범위를 허용할 수 있습니다. 예를 들어 아래의 `^18.3.1`은 React 18 버전대의 더 최신 호환 버전이 설치될 여지를 둡니다.

```json
"react": "^18.3.1"
```

`package-lock.json`은 그중 **이번 프로젝트에서 실제로 선택된 정확한 버전**과, React·Vite가 내부적으로 사용하는 모든 하위 라이브러리 버전까지 기록합니다.

- `npm install`을 처음 실행하거나 라이브러리를 바꾸면 npm이 자동으로 만들고 갱신한다.
- 직접 작성하거나 직접 고치는 파일이 아니다.
- 없더라도 `npm install`은 가능하지만, 설치 시점에 따라 조금 다른 버전 조합이 선택될 수 있다.
- 있으면 다른 PC에서도 같은 라이브러리 조합을 설치하기 쉬워진다.

### 설정 파일은 누가 만드는가?

대부분은 처음 프로젝트를 만들 때 도구가 자동으로 생성합니다.

| 파일 | 주로 만드는 주체 | 개발자가 직접 만지게 되는 때 |
|---|---|---|
| `package.json` | Vite 프로젝트 생성 도구 또는 `npm init` | 라이브러리·실행 명령을 추가할 때 |
| `package-lock.json` | `npm install` | 보통 직접 수정하지 않음 |
| `tsconfig.json` | TypeScript/Vite 템플릿 | TypeScript 검사 규칙을 바꿀 때 |
| `vite.config.ts` | Vite 템플릿 | 프록시, 포트, 플러그인을 설정할 때 |

지금은 이 파일들을 외우는 것보다, `src` 안의 `.tsx` 파일에서 컴포넌트·상태·이벤트를 읽는 데 시간을 쓰는 편이 훨씬 좋습니다.

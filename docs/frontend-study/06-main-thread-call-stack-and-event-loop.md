...
| **HTML 파서** | HTML을 읽어 DOM을 만들고, `script` 요소의 실행 조건이 맞으면 JavaScript 엔진에 스크립트 실행을 요청한다. |
...
| **렌더링 태스크** | 렌더링 기회가 생겼을 때 준비되는 태스크다. 이벤트 루프가 다른 태스크와 마찬가지로 선택하면 같은 렌더러 메인 스레드가 스타일·레이아웃·페인트 준비 등의 렌더링 관련 코드를 실행한다. |
...
flowchart TB
  ...
  subgraph RENDERER[" "]
    ...
    subgraph MAIN["렌더러 메인 스레드(OS 스레드 하나)"]
      ...
      MICRO_JS["큐가 비어 있지 않으면<br/>마이크로태스크를 하나씩 실행<br/>JavaScript 콜백이면 엔진 호출<br/>새로 추가된 것까지 큐가 빌 때까지 반복"]
...
flowchart TB
  subgraph MAIN["선택된 태스크를 실행 중인 렌더러 메인 스레드"]
    direction TB
    TASK["선택된 태스크 코드 실행<br/>(태스크 종류별 대표 시작 경로<br/>분기들은 완전히 배타적이지 않음)"]
    ...
    RENDER_CODE["렌더링 갱신 단계<br/>rAF 콜백이 있으면 JavaScript 엔진 호출<br/>이후 스타일 · 레이아웃 · 페인트 준비"]
...
그림의 분기는 **태스크 종류별 대표 시작 경로**이며, 한 태스크가 오직 한 종류의 코드만 실행한다는 뜻은 아니다. HTML 파싱 태스크가 JavaScript 엔진을 호출했다가 파서로 돌아올 수 있고, 렌더링 태스크도 `requestAnimationFrame` 콜백을 실행하기 위해 JavaScript 엔진을 호출한 뒤 스타일·레이아웃 등의 단계를 이어 갈 수 있다.
...
이벤트 루프 구현 코드가 실행 가능한 태스크를 선택하면 같은 렌더러 메인 스레드의 실행 제어권이 선택된 태스크 코드로 이동한다. 현재 실행 중인 태스크의 단계는 HTML 파서 코드를 실행하거나 JavaScript 엔진에 직접 코드 실행을 요청할 수 있다. HTML 파서도 같은 태스크 안에서 JavaScript 엔진에 스크립트 실행을 요청할 수 있다.

JavaScript 실행이 끝나면 필요한 정리 과정을 거쳐 JavaScript 엔진을 호출한 파서 또는 태스크 단계로 돌아가 처리를 이어 간다. 실행 컨텍스트 스택이 비는 경우처럼 표준이 정한 조건에서는 이 정리 과정에서 마이크로태스크 체크포인트가 수행된다. 태스크의 모든 처리가 끝나면 태스크 종료 뒤의 체크포인트를 수행하고 이벤트 루프 구현 코드로 돌아간다.
...
| JavaScript 코드 실행과 콜 스택 관리 | JavaScript 엔진 |
| 스타일·레이아웃·페인트 준비 | 렌더링 관련 코드 |

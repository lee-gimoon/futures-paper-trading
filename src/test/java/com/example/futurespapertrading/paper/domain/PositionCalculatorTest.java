...
class PositionCalculatorTest {
    ...
    void 진입_run의_주문레버리지를_같은방향_추가에도_유지() {
        ...
        assertEquals(20, PositionCalculator.openLeverage(fills, Map.of(1L, 20, 2L, 5), 10));
    }

    @Test
    void 포지션이_닫히면_fallback_레버리지() {
        List<PaperFill> fills = List.of(fillO(1L, "BUY", "100", "1"), fillO(2L, "SELL", "110", "1"));
        assertEquals(7, PositionCalculator.openLeverage(fills, Map.of(1L, 20, 2L, 5), 7)); // flat → fallback
    }
    ...
    void 뒤집히면_뒤집은_주문의_레버리지() {
        ...
        assertEquals(5, PositionCalculator.openLeverage(fills, Map.of(1L, 20, 2L, 5), 10));
    }
    ...
    // orderId를 지정하는 변형 (openLeverage가 주문별 레버리지 맵을 orderId로 찾으므로).

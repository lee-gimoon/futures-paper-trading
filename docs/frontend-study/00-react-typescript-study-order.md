...
## 처음에는 깊게 공부하지 않아도 되는 파일

아래 파일들은 프론트엔드 프로젝트를 실행하는 데 필요하지만, React 입문 단계에서 직접 작성하거나 내부를 완벽히 이해할 필요는 없습니다. **역할만 알고 넘어가세요.**

| 파일/폴더 | 지금 알아둘 역할 | 지금 할 일 |
|---|---|---|
| `frontend/node_modules/` | npm이 실제로 내려받아 설치한 React, Vite 등의 라이브러리 폴더 | 열어보거나 수정하지 않기 |
| `frontend/package-lock.json` | 실제로 설치할 라이브러리와 하위 라이브러리의 정확한 버전을 기록한 잠금 목록 | 열거나 직접 수정하지 않기 |
| `frontend/package.json` | 프로젝트가 사용할 라이브러리와 실행 명령을 적은 목록 | `scripts`, `dependencies`만 가볍게 보기 |
| `frontend/tsconfig.json` | `src`의 TypeScript 검사 규칙 | 오류가 날 때 나중에 보기 |
| `frontend/tsconfig.node.json` | `vite.config.ts` 같은 Node.js 쪽 파일의 TypeScript 검사 규칙 | 지금은 넘어가기 |
| `frontend/vite.config.ts` | Vite 개발 서버, 포트, 백엔드 프록시 설정 | 프록시가 필요할 때 보기 |
| `frontend/index.html` | React가 화면을 그리기 시작할 `root` 공간 | `<div id="root">`만 한 번 확인 |
| `frontend/dist/` | `npm run build`가 만든 배포 결과물 | 직접 수정하지 않기 |
| `frontend/.gitignore` | Git에 올리지 않을 파일 목록 | 지금은 넘어가기 |
...
|---:|---|---|---|
| 1 | `frontend/src/main.tsx` | React 앱의 시작점, `root`에 `<App />`을 그리는 방법 | `StrictMode`의 내부 동작 |
| 2 | `frontend/src/App.tsx` | 컴포넌트를 조립하는 방법, `useState`, 조건부 렌더링, props를 전달하는 모습 | 차트·주문 전체 로직 |
| 3 | `frontend/src/auth/components/LoginForm.tsx` | JSX, 컴포넌트, props, `input`, `button`, `onClick`/`onSubmit` 이벤트 | API가 실제로 요청되는 세부 과정 |
| 4 | `frontend/src/shared/types.ts` | TypeScript `type`, 문자열 유니온, `null`을 포함한 타입 | 모든 도메인 타입의 의미 |
| 5 | `frontend/src/auth/api/authApi.ts` | `fetch`, `async`/`await`, 서버 요청과 응답 | 쿠키·HTTP 헤더의 상세 규칙 |
| 6 | `frontend/src/auth/hooks/useAuth.ts` | 커스텀 Hook, 상태를 여러 컴포넌트에서 쓰기 좋게 묶는 방법 | 로그인 API의 모든 예외 처리 |
| 7 | `frontend/src/market/components/OrderBook.tsx` | 배열을 `map`으로 화면 목록으로 바꾸기, props로 함수 전달하기 | SSE 연결 방식 |
| 8 | `frontend/src/market/hooks/useOrderBookStream.ts` | `useEffect`, 실시간 데이터 구독, cleanup(연결 해제) | EventSource의 프로토콜 세부 사항 |
| 9 | `frontend/src/market/engine/quote.ts` | React 밖의 순수 함수와 화면 코드의 분리 | 호가 계산 공식 자체 |
| 10 | `frontend/src/paper/components/OrderForm.tsx` | 폼 입력 상태, 선택값, 부모에게 주문 요청을 전달하는 방법 | 주문 도메인 규칙 전체 |
| 11 | `frontend/src/paper/hooks/useTrading.ts` | 여러 API 호출·로딩·오류·데이터 상태를 하나의 Hook으로 관리 | 각 API의 구현 세부 |
| 12 | `frontend/src/paper/components/TradingPanel.tsx` | 작은 컴포넌트를 조합해 기능 화면을 만드는 방법 | CSS 레이아웃 미세 조정 |
| 13 | `frontend/src/market/components/CandleChart.tsx` | 외부 차트 라이브러리를 React 컴포넌트 안에서 사용하는 방법 | 차트 라이브러리 전체 API |
...
### 3. `frontend/src/auth/components/LoginForm.tsx`
...
### 4. `frontend/src/shared/types.ts`
...
### 5. `frontend/src/auth/api/authApi.ts`
...
### 6. `frontend/src/auth/hooks/useAuth.ts`
...
### 7. `frontend/src/market/components/OrderBook.tsx`
...
### 8. `frontend/src/market/hooks/useOrderBookStream.ts`
...
### 9. `frontend/src/market/engine/quote.ts`
...
### 10. `frontend/src/paper/components/OrderForm.tsx`
...
### 11. `frontend/src/paper/hooks/useTrading.ts`
...
### 12. `frontend/src/paper/components/TradingPanel.tsx`
...
### 13. `frontend/src/market/components/CandleChart.tsx`

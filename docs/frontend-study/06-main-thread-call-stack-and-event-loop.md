...
이벤트 루프는 실행 순서를 정하는 처리 모델이고, 렌더러 메인 스레드는 그 처리에 필요한 코드를 실제로 실행하는 OS 스레드다. 두 대상의 관계를 단순화하면 다음과 같다.

```text
렌더러 메인 스레드가 브라우저의 이벤트 루프 구현 코드를 실행
└─ 페이지가 실행되는 동안 내부 처리 반복
   ├─ 실행 가능한 태스크 확인·선택
   ├─ 선택한 태스크의 처리 코드 실행
   │  └─ JavaScript 실행이 필요하면 V8 호출
   │     └─ 함수 호출 정보가 콜 스택에 쌓이고 종료되면 제거
   ├─ 선택한 태스크가 끝나면 마이크로태스크 체크포인트 수행
   │  └─ 준비된 Promise 반응·queueMicrotask 콜백 등을 실행
   ├─ 렌더링 기회가 생기면 렌더링 갱신 태스크도 실행 대상으로 준비
   │  └─ 그 태스크가 선택되면 메인 스레드가 렌더링 갱신 단계 수행
   └─ 다음 태스크 선택 반복
```

#### **이벤트 루프와 메인 스레드의 역할**

**이벤트 루프는 실행 가능한 태스크의 선택·실행과 마이크로태스크 체크포인트 등의 처리 순서를 정의한 모델이며, 브라우저 내부의 스케줄링 코드로 구현된다. 렌더러 메인 스레드는 이 구현 코드를 반복 실행하는 실제 OS 스레드다. 이벤트 루프 구현 코드는 다음 태스크를 선택하고 필요한 처리 단계를 호출하며, 선택된 태스크를 처리하면서 호출되는 Blink·V8·JavaScript 콜백 코드도 같은 렌더러 메인 스레드가 실행한다. 콜 스택은 V8이 현재 실행 중인 JavaScript 함수 호출을 관리하는 실행 상태이며, 코드를 실행하는 별도 스레드가 아니다.**

- **이벤트 루프:** 무엇을 다음에 처리할지 정하는 처리 모델과 브라우저 내부 구현 로직
- **렌더러 메인 스레드:** 이벤트 루프 구현 코드와 선택된 태스크 코드를 실제로 실행하는 OS 스레드
- **태스크·마이크로태스크 큐:** 실행을 기다리는 작업의 저장소
- **V8:** JavaScript를 실행하는 엔진
- **콜 스택:** V8이 현재 실행 중인 함수 호출을 기록하는 실행 상태
...
flowchart TB
  subgraph MAIN["선택된 태스크를 실행 중인 렌더러 메인 스레드"]
    direction TB
    TASK["선택된 태스크 코드 실행<br/>(태스크 종류에 따라<br/>처리 흐름이 달라짐)"]
...
**Blink와 V8:** Blink는 Chromium의 웹 플랫폼·렌더링 엔진으로 HTML 파싱, DOM·웹 API, 이벤트 전달, 스타일·레이아웃·페인트 준비 등을 담당한다. V8은 JavaScript 실행과 콜 스택을 담당하는 JavaScript 엔진이다. 둘은 태스크 종류가 아니며, 하나의 태스크 안에서 Blink가 V8을 호출한 뒤 다시 Blink 코드로 돌아올 수 있다.

**메시지 펌프:** Chromium 내부에서 태스크 스케줄러와 OS의 대기·깨움 기능을 연결하는 코드다. 실행 가능한 태스크가 없으면 OS 대기 기능을 호출해 렌더러 메인 스레드를 대기 상태로 두고, 태스크 게시·입력·IPC·타이머 시각 도달 등으로 대기가 해제되면 이벤트 루프 처리를 재개한다. 따라서 일이 없는 동안 태스크 큐를 계속 확인하며 CPU를 소비하지 않는다.

**마이크로태스크 체크포인트:** 태스크가 끝나면 **마이크로태스크 체크포인트 자체는 수행**한다. 큐가 비어 있으면 실행할 마이크로태스크 없이 끝나고, 비어 있지 않으면 실행 중 새로 추가된 것까지 큐가 빌 때까지 처리한다. 체크포인트는 태스크 종료 외에도 HTML 표준이 정한 시점에 수행될 수 있다.

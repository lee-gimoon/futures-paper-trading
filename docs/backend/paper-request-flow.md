...
```text
이미 포지션이 열려 있는 상태에서 추가 주문이 들어와도,
이번 주문의 fill만 따로 계산하지 않는다.
해당 회원의 전체 fill history를 다시 처음부터 누적해서
현재 포지션 상태를 만드는 방식이다.
```

예를 들어 어떤 회원이 BTC를 10년 동안 계속 매수/매도했다면,
현재 포지션을 화면에 보여주기 위해 그 회원의 10년치 체결 내역을 다시 조회하고
처음부터 replay해야 한다.
...
```text
계산 기준이 단순하다.
항상 fill history 전체를 replay하므로 결과가 일관적이다.
별도의 현재 포지션 저장 테이블이 없어도 된다.
```

문제점:

```text
체결 내역이 많아질수록 DB 조회량이 늘어난다.
collectList() 때문에 많은 fill이 한 번에 메모리에 올라간다.
compute(fills)가 매번 처음부터 반복 계산하므로 CPU 사용량이 늘어난다.
포지션 화면/API 응답이 느려질 수 있다.
```

WebFlux가 기본으로 사용하는 Reactor Netty 이벤트 루프 관점에서
특히 문제가 될 수 있는 지점은 두 개다.
...
정리하면 DB 조회는 reactive지만, 포지션 계산은 reactive가 아니다.
...
부담 = 사용자별 체결 이력 크기 x 동시 접속/요청 수
```

결론은, 학습용/소규모에서는 괜찮은 단순한 방식이다.  
하지만 체결 이력이 많아지고 사용자가 많아지는 실서비스 규모에서는
DB, 메모리, CPU, 응답속도 측면에서 문제가 생길 수 있다.

실서비스식 개선 방향은 보통 이렇다.

```text
체결이 발생할 때마다 현재 포지션 상태를 DB에 업데이트한다.
화면에서는 전체 fill history가 아니라 저장된 현재 포지션만 조회한다.
```

...
```mermaid
%%{init: {"themeVariables": {"fontSize": "24px"}, "flowchart": {"nodeSpacing": 35, "rankSpacing": 45}}}%%
flowchart TB
  COMPONENTS["브라우저 측 구성 요소<br/>(렌더러 프로세스 바깥)<br/>브라우저 프로세스 · 네트워크 서비스<br/>HTML · 입력 · 네트워크 완료 전달"]
...
> 두 번째 그림의 맨 위 **브라우저 측 구성 요소** 박스는 OS가 아니라, 브라우저 애플리케이션 안에서 렌더러 프로세스 바깥에 있는 브라우저 프로세스·네트워크 서비스다. 네트워크 서비스는 브라우저 구현에 따라 별도 프로세스에서 동작할 수 있다.
>

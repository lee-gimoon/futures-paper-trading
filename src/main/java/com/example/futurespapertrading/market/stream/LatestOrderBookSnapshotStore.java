...
public class LatestOrderBookSnapshotStore {
	...
	// 새 호가 snapshot이 발행될 때마다 외부 코드가 받아볼 수 있는 Flux를 제공한다.
	// sink.asFlux()는 Sink의 발행 흐름을 구독 가능한 Flux 뷰로 노출한다.
	// 실제 구독은 컨트롤러/SSE, PendingOrderMatcher, LiquidationMonitor 쪽에서 이루어진다.

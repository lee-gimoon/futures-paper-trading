...
```mermaid
%%{init: {"themeVariables": {"fontSize": "21px"}, "flowchart": {"nodeSpacing": 55, "rankSpacing": 65}}}%%
flowchart TB
  subgraph BROWSER["브라우저 애플리케이션"]
    COMPONENTS["렌더러 프로세스 바깥의 브라우저 구성 요소<br/>브라우저 프로세스 · 네트워크 서비스<br/>HTML 수신 · 사용자 입력 전달 · 네트워크 완료 전달"]
...
> 두 번째 그림의 맨 위 구성 요소는 OS가 아니라 **브라우저 애플리케이션 안에서 렌더러 프로세스 바깥에 있는 브라우저 프로세스·네트워크 서비스**다. 네트워크 서비스는 브라우저 구현에 따라 별도 프로세스에서 동작할 수 있다.
>

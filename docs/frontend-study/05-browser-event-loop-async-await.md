...
>
> `0`은 콜백을 지금 즉시 호출하라는 뜻이 아니라, 타이머에 요청하는 지연값이다. `setTimeout`은 콜백을 브라우저 타이머에 등록하고 바로 반환하므로 다음 동기 코드가 계속 실행된다. 지연 시간이 지나면 콜백을 실행할 **타이머 태스크**가 큐에 등록된다.
>
> `() => {}` 자체는 단지 함수이며, 어느 큐에도 자동으로 등록되지 않는다. 태스크와 마이크로태스크 중 어디에서 실행될지는 화살표 함수 문법이 아니라 **어떤 기능이 그 코드를 나중에 실행하도록 예약했는지**에 따라 결정된다.
>
> - `setTimeout(() => {}, 0)` → 타이머 태스크
> - `Promise.resolve().then(() => {})` → 마이크로태스크
> - `queueMicrotask(() => {})` → 마이크로태스크
> - `await` 다음 코드 → 기다리던 Promise가 완료되면 마이크로태스크로 재개
>
> JavaScript는 현재 실행 중인 동기 코드를 호출 스택이 빌 때까지 끝내며, 그 중간에 타이머 콜백이나 마이크로태스크가 끼어들지 않는다. 이것이 **run-to-completion**이다. 따라서 위 예시에서는 `1 → closePanel() 내부 코드 → 2`까지 먼저 실행된다. 그 후 마이크로태스크가 있다면 먼저 처리하고, 이벤트 루프가 타이머 태스크를 선택했을 때 `3`이 실행된다.

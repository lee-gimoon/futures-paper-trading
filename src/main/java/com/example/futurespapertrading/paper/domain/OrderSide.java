...
public enum OrderSide {
    ...
    SELL
    // ── 보통 이 enum을 쓰는 방식 ──
    // OrderSide side = OrderSide.BUY;   // BUY 상수를 변수에 담기
    // OrderSide.BUY.name();             // "BUY" 문자열 반환
    // OrderSide.valueOf("BUY");         // "BUY" 문자열을 OrderSide.BUY로 변환
    // side == OrderSide.BUY;            // enum 비교는 보통 == 사용
    // System.out.println(side);         // BUY 출력(toString()이 자동 호출됨)
}

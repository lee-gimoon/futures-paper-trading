...
// TradingPage의 화면 역할과 컴포넌트 계층
// - TradingPage는 상단 인증 바, 공개 시세 화면, 로그인 후 거래 화면을 조립하는 거래 페이지 컴포넌트다.
...
// 로그인했을 때만 표시할 거래 레이아웃이 부모(TradingPage)로부터 받는 값들.
...
// 로그인 상태일 때 공통 차트·호가창에 사용자 전용 거래 패널과 주문·체결 내역을 추가해 3열로 배치하는 보조 컴포넌트.
// TradingPage는 인증과 공개 시세를 관리하고, 이 컴포넌트는 사용자별 거래 데이터를 조회해 표시한다.
function AuthenticatedTradingLayout({
  ...
  // 이전 렌더링과 orders 배열이 같으면 계산 결과를 재사용하도록 useMemo를 사용한다.
      ...
      <div className="chart-col">
        ...
        <div className="chart-activity">
          <section className="activity-panel pending-orders">
            <h3>대기 주문</h3>
            {/* OrderList에서 주문 취소가 성공하면 refresh를 호출해 거래 데이터를 다시 가져온다. */}
            <OrderList
              orders={openOrders}
...
export default function TradingPage() {
  // 이 Hook은 TradingPage에 연결된 snapshot 상태의 현재 값을 반환한다.
  // Hook 내부의 setSnapshot(...)이 상태 갱신을 예약하면 React는 TradingPage를 다시 렌더링하고,
  ...
  // useAuth() 반환 예시: { user: { id: 1, email: "user@example.com", displayName: "사용자" }, loading: false, error: null, login, signup, logout, expireSession }
  ...
  // setForm으로 이 값을 바꾸면 TradingPage가 다시 렌더링되어 아래 조건부 JSX가 해당 폼을 표시하거나 제거한다.
  ...
  return (
    // trading-page는 기존 body가 담당하던 전체 화면 배경·글꼴·바깥 여백을 맡는다.
    // 그 안의 app은 기존과 동일하게 콘텐츠 너비를 최대 1600px로 제한하고 가운데 정렬한다.
    <div className="trading-page">
      ...
      <div className="layout">
        ...
        {/* TradingPage가 부모로서 시세 데이터와 이벤트 처리 함수를 AuthenticatedTradingLayout에 props로 전달한다.
            `snapshot={snapshot}`에서 왼쪽 snapshot은 자식 컴포넌트가 받을 props 이름이고,
            오른쪽 {snapshot}은 TradingPage 안에서 useOrderBookStream으로 얻은 JavaScript 변수 값이다. */}

/**
 * 이 파일은 호가 항목 하나의 표시 방식을 정의하는 재사용 컴포넌트다.
 * FlatList는 각 호가 데이터마다 같은 PriceRow를 반복해서 사용한다.
 * 부모 화면이 가격·수량·매수/매도 호가 구분을 props로 전달하면
 * PriceRow는 전달받은 값과 구분에 맞는 색상을 화면에 표시한다.
 */
...
type PriceRowProps = {
  ...
  side: PriceSide;
  // 부모가 함수를 전달하면 호가 행을 눌러 해당 가격을 선택할 수 있다.
  onPress?: () => void;
};

// 구조 분해로 props의 가격, 수량과 방향을 각각 꺼낸다.
export function PriceRow({ price, quantity, side, onPress }: PriceRowProps) {

...
>
> **태스크(task)**는 브라우저 이벤트 루프가 처리하는 작업 단위이지, JavaScript 함수 하나를 뜻하지는 않는다. 브라우저가 현재 태스크를 처리하다 JavaScript를 실행하기 시작하면, 그 JavaScript의 호출 스택이 빌 때까지 클릭·타이머 같은 **다른 태스크**를 중간에 실행하지 않는다. 이것이 **run-to-completion**이다.
>
> 태스크가 수행하는 작업과 그 안에서 실행될 수 있는 JavaScript의 예는 다음과 같다.
>
> | 태스크 작업 | 그 안에서 실행될 수 있는 JavaScript |
> | --- | --- |
> | 클릭 이벤트 전달 | 등록한 `click` 이벤트 핸들러 |
> | 타이머 만료 처리 | `setTimeout` 콜백 |
> | 네트워크 응답 처리 | `fetch` Promise 완료 처리. `.then`과 `await` 다음 코드는 마이크로태스크로 실행 |
> | HTML 파싱 작업 | `<script>`를 만나면 해당 스크립트 실행 |
>
> 따라서 예시의 실행 순서는 `1 → closePanel() 내부 코드 → 2 → 3`이다. `closePanel()`은 같은 클릭 태스크 안에서 즉시 호출된다. 반면 `setTimeout`은 타이머 콜백을 등록할 뿐이다. 시간이 `0`이어도 현재 `handleClick`의 동기 코드가 끝나고, 마이크로태스크까지 처리된 뒤에야 타이머 태스크가 실행되어 `3`을 출력할 수 있다.

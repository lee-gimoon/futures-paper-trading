...
| 층 | 책임 |
|---|---|
| `LoginForm` | 로그인 화면을 보여 주고 이메일·비밀번호 입력과 제출 상태를 관리한다. |
| `useAuth` | 로그인·로그아웃 같은 인증 작업을 처리하고, 현재 사용자 state를 갱신한다. |
| `authApi` | 서버 API 주소와 HTTP 요청 세부 사항을 관리하고 실제 요청을 보낸다. |

즉 `LoginForm`은 요청하고, `useAuth`는 인증 흐름을 처리하며, `authApi`는 서버와 통신한다.
